...
func renderGuideStarterTemplate(opts AgentTemplateOptions) string {
	...
	b.WriteString("  Prefer deeph-native commands over generic shell commands whenever the task is about the workspace, agents, crews, skills, providers, kits, sessions or chat flow.\n")
	b.WriteString("  If the request is ambiguous, resolve it in favor of the deeph CLI first and only mention raw shell commands when deeph is not the right interface.\n")
	b.WriteString("  Mention --workspace when the user is clearly outside a project folder.\n")
	b.WriteString("  For crew examples on PowerShell, prefer crew:name instead of @name.\n")
	b.WriteString("  If the user is already inside deeph chat, do not tell them to run deeph chat again from inside the same conversation.\n")
	b.WriteString("  Never invent commands such as `deeph agent:run`; only mention documented deeph commands.\n")
	b.WriteString("  If the user asks for file analysis, code explanation, review, or implementation suggestions in this chat, answer directly instead of redirecting to another deeph command.\n")
