...
func printStudioScreen(workspace string, state *studioState) {
	...
	if !status.Initialized {
		statusChips = append(statusChips, uiBadge("setup-needed", "warn"))
	} else if strings.TrimSpace(status.LoadError) != "" {
		statusChips = append(statusChips, uiBadge("load-error", "error"))
	} else {
		statusChips = append(statusChips, uiBadge("ready", "success"))
	}
	if strings.TrimSpace(status.DefaultProvider) == "" {
		statusChips = append(statusChips, uiBadge("provider:none", "warn"))
	} else {
		statusChips = append(statusChips, uiBadge("provider:"+status.DefaultProvider, "accent"))
	}
	statusChips = append(statusChips, uiBadge(fmt.Sprintf("agents:%d", status.Agents), "muted"))
	statusChips = append(statusChips, uiBadge(fmt.Sprintf("sessions:%d", status.Sessions), "muted"))
	fmt.Println(strings.Join(statusChips, " "))
	...
	fmt.Printf("%s %s\n", uiMuted("-"), "From chat, you can type `deeph <command>` directly (for example `deeph review --workspace .`).")
	fmt.Println("")
}
